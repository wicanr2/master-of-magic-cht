...
## 結論(先講):UI 是忠實的,差在「顯示層」不是「介面設計」

**實際截圖一比就清楚:重製版的介面美術與佈局,和原版 DOS 幾乎一模一樣**——因為重製版直接讀原版的
`.lbx` 資產來畫。逐畫面(選巫師、選種族、大地圖、施法場景)看下來,選單列、面板、地圖磚、巫師肖像、
金色標題字、右側 HUD 配置全都對得上,**不是被重新設計過的介面**。所以「沒 DOS 感」**不是因為介面長得不一樣**。
...
截圖目視到的**介面層差異只有細節**:① 頂部選單列按鈕的間距/留白略不同(DOS 七顆鈕緊貼、重製版較分散);
② 右側 HUD 中段內容隨狀態變(沒選單位時 DOS 顯示金/食/法力產出 + Next Turn,重製版選了單位時顯示單位列 + 完成/巡邏/等待/建造)——這是**同一套 UI 的不同狀態,不是改版**。字型也忠實(英文沿用 LBX 點陣字,常被誤會,見下)。
...
### 4. 版面由引擎重畫,但截圖比對下「佈局忠實、僅細節位移」
- **原版**:固定點陣構圖。
- **重製版**:讀原始 sprite/底圖,版面由引擎重新排版繪製(部分畫面用原 baked 底圖、其餘逐元素重畫)。
- **截圖比對結果(已比對)**:選巫師/選種族/大地圖/施法場景**佈局與原版一致**;唯一明顯的版面差異是
  **頂部選單列七顆按鈕的間距**(DOS 緊貼成一排、重製版較分散有間隙)。其餘元素對齊、面板配置都對得上。
  互動行為有些不同(如開發者承認 minimap 不可點、縮放層級是後加的),但那是操作不是視覺。
- **為何沒 DOS 感**:選單間距這種細節**影響很小**;主要落差仍是第 1~3 點的顯示層。
- **可信度:高(已比對截圖)**。
...
- **「介面被重新設計、跟原版不一樣」**:**截圖逐畫面比對下,UI 美術與佈局幾乎一致**(重製版讀原版 LBX);
  違和感來自顯示層(長寬比 / CRT / 音色),不是介面改版。只有頂部選單按鈕間距是看得出的小差別。
