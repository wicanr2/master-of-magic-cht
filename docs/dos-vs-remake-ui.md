...
截圖目視到的**介面層差異只有細節**:右側 HUD 中段內容隨狀態變(沒選單位時 DOS 顯示金/食/法力產出 + Next Turn,
重製版選了單位時顯示單位列 + 完成/巡邏/等待/建造)——這是**同一套 UI 的不同狀態,不是改版**。字型也忠實(英文沿用 LBX 點陣字,常被誤會,見下)。
...
- **截圖比對結果(已比對)**:選巫師/選種族/大地圖/施法場景**佈局與原版一致**;連頂部七顆選單鈕也是
  原版座標(`MakeHudUI()` 硬定位、對齊原版 HUD 底圖,見上方「更正」框),**沒有可辨識的版面差異**。
...
- **「介面被重新設計、跟原版不一樣」**:**截圖逐畫面比對下,UI 美術與佈局一致**(重製版讀原版 LBX);
  違和感來自顯示層(長寬比 / CRT / 音色),不是介面改版。連頂部選單按鈕間距也是原版座標(見上方更正框)。
