...
把各畫面用的原版 LBX 與座標列出來,方便逐項核對(原始碼依據 `game/magic/` 各檔)。
「沒 DOS 感」與「跟原版不一樣」的疑問,多半能在這張表找到答案是「狀態差/不同畫面」而非改版。

| 畫面 | 原版資產 | 忠實? | 備註 / 常見誤會 |
|---|---|---|---|
| 開場動畫 + 標題卡 | `intro.lbx`(逐格照播)| ✅ | 金字「Master of Magic」卡是這裡的一格,**不是**主選單 |
| 主選單(城堡底圖)| `mainscrn.lbx` 0/5 | ✅(底圖)| **疊了重製版製作群字幕「2025 / kazzmir…」(`mainview/view.go:207-213`,原版無)= 唯一真改動** |
| 遊戲內 頂部選單列 | `main.lbx` 0 + 1–7 | ✅ 原版座標 | 按鈕硬定位 `7,47,89,140,184,226,270`(`MakeHudUI`)|
| 右側 HUD(**沒選單位**)| `main.lbx` 34 + Next Turn | ✅ 原版座標 | 小地圖→庫存→金/食/法力收入→Next Turn(座標見 §4)|
| 右側 HUD(**選了單位**)| `main.lbx` 8–11、24 | ✅ | 改顯示單位列 + 完成/巡邏/等待/建造 ← 拿這個比「沒選單位的 DOS 圖」會誤判 |
| 選巫師 / 選種族 / 城市 / 戰鬥 / 施法 | 各原版 LBX | ✅ | 逐畫面截圖比對一致 |

> 一句話:**遊戲內與開場全用原版資產、原版座標;唯一的非原版介面元素是主選單那段製作群字幕。**
> 其餘「不一樣」的感覺來自顯示層(長寬比 / CRT / 音色,見下)或「不同狀態 / 不同畫面」的比錯對象。
